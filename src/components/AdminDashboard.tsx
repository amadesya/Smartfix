...
export function AdminDashboard() {
  const [activeTab, setActiveTab] = useState<'tickets' | 'services' | 'users' | 'reports' | 'contacts'>('tickets');
...
╚═══════════════════════════════════════════════════════════╝`}
        ...
        {activeTab === 'tickets' && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 border-b-2 border-[#00ff00]">
        ...
        <div className="border-b-2 border-[#00ff00] flex flex-wrap">
          <TabButton
        ...
        <div className="p-4 md:p-6">
          ...
          {activeTab === 'reports' && <ReportsGenerator tickets={tickets} />}
...
function StatCard({ label, value, color = 'text-[#00ff00]' }: StatCardProps) {
  return (
    <div className="border-2 border-[#00ff00] p-3 text-center">
      <div className="text-xs opacity-70 mb-2">{label}</div>
      <div className={`text-2xl md:text-3xl ${color}`}>{value}</div>
    </div>
  );
}
...
function TabButton({ active, onClick, label }: TabButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`flex-1 py-3 px-4 transition-colors text-sm md:text-base ${
        active
